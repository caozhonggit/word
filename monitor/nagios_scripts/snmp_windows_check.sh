...
#snmp_windows_check.sh -H hostname/IP -c community -w warning -c critical -t [mem|cpu|user|process]
#This plagin is same as "snmp_aix_check.sh" except "uptime" portion. 
...
print_usage() {
        echo "Usage: "
		echo "  $PROGNAME -H HOST -C community -w warning -c critical -t [mem|user|cpu|process]"
