...
#2011-12-19  建立此版本，原因是有人需要jdbc大于某值时告警，而原来的jdbc默认有等待就告警仍为默认判断办法。
...
print_usage() {
		...
		echo " 		$PROGNAME [-v 1|2c] -H HOST -C community [-p port] -t Weblogic_ServerName -c critical -s jdbc"
		echo "Check Weblogic Hogging Thread:"
		echo "		$PROGNAME [-v 1|2c] -H HOST -C community [-p port] -t Weblogic_ServerName -s hoggingthread"
		echo " "
}
...
if [[ -n $HOSTNAME && -n $COMMUNITY && -n $PR && -n $TITLE ]];then 
	case $PR in 
		...
		jvm)
			...
			if [[ -n $jvm_cur_free && -n $jvm_cur_size && "$jvm_cur_size" != "Such" ]];then
		...
		jdbc)
			...
			if [[ -n $CRIT ]];then
				if [[ "$r1" -eq 0 && "$jdbc_pool_g" != "Such"  && "$jdbc_pool_g" != "jdbcConnectionPoolRuntimeName=NoSuchObjectavailableonthisagentatthisOID" ]];then
					n=0
					crit_num=0
					for i in "${jdbc_pool_g[@]}";do
						if [[ ${jdbc_active_count_g[$n]} -gt $CRIT ]];then
							printf "%s%s%s%s%s" " <= CRIT - JDBC Pool : " ${jdbc_pool_g[$n]} " - Current Active Connections Num:" ${jdbc_active_count_g[$n]} "=>" 
							(( crit_num=${crit_num}+1 ))
						else		
							printf "%s%s%s%s%s" " <= JDBC Pool : " ${jdbc_pool_g[$n]} " - Current Active Connections Num:" ${jdbc_active_count_g[$n]} "=>"         						
						fi
						(( n=${n}+1 ))			
					done
		...
		hoggingthread)
			if [[ -n $PORT ]];then
				hogging_thread=$( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME}:${PORT} $threadPoolRuntimeHoggingThreadCount | gawk '{print $4}' )
			else
				hogging_thread=$( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME} $threadPoolRuntimeHoggingThreadCount | gawk '{print $4}' )
			fi
