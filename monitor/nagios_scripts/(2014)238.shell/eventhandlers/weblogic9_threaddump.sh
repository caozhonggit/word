...
#This script and WLSThreadsMonitor.class is the same as weblogic10_threaddump.sh and WLSThreasMonitor.class.
#当weblogic9队列超过了nagios的告警阀值，产生严重告警时，会触发此脚本（此脚本作为nagios的事件触发来使用）
#此脚本会通过调用Nagios服务器上的WLSThreadsMonitor.class java程序来获取远程weblogic9的threaddump
...
#          define command{
#					        command_name                    weblogic9_threaddump
#				    	    command_line                    $USER1$/eventhandlers/weblogic9_threaddump.sh -H $HOSTADDRESS$ -S $ARG1$ -P $ARG2$ -u $ARG3$ -p $ARG4$ -s $SERVICESTATE$ -t $SERVICESTATETYPE$ -n $SERVICEATTEMPT$
#						}
#
#三、 在要触发此事件命令的服务检查项中添加如下定义：
#					event_handler                   weblogic9_threaddump!PerServer!8001!weblogic!Weblogic123
...
DumpDir="/srv/weblogic_thread_dump/9"
PROGNAME=`basename $0`
		...
		CRITICAL)
			case "${NAGIOS_STATETYPE}" in
				SOFT)
					...
					;;
				HARD)
					cd /usr/local/nagios/libexec/eventhandlers/weblogic9
