...
#				    	    command_line                    $USER1$/eventhandlers/nagios_eventhandle_weblogic_jstack.sh -H #$HOSTADDRESS$ -s $SERVICESTATE$ -t $SERVICESTATETYPE$ -n $SERVICEATTEMPT$
#						}
#
#三、 在要触发此事件命令的服务检查项中添加如下定义：
#					event_handler                   weblogic_jstack_snapshot
...
print_usage() {
        echo "Usage: "
        echo "  $PROGNAME -H HOST -s SERVICESTATE -t SERVICESTATETYPE -n SERVICEATTEMPT"
		echo " "
        echo "Get event queue of weblogic server through t3 when assgined nagios state is happened"
		echo " "
		echo "eg:    "
		echo "  $PROGNAME -H 10.1.8.74  -s CRITICAL -t HARD -n 1"
}
		...
		CRITICAL)
			case "${NAGIOS_STATETYPE}" in
				...
				HARD)
					...
					/usr/local/nagios/libexec/check_nrpe -H ${WEBLOGIC_HOSTNAME} -t 30 -c jstack_dump 
					;;
			esac
			;;
	esac
