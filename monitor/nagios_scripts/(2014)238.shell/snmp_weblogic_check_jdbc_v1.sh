	...
	case $PR in 
		...
		jdbcstat)
			...
			if [[ -n $PORT ]];then
				...
				jdbc_pool_stat_li=$( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME}:${PORT} $jdbcConnectionPoolRuntimeState | wc -l)
				if [[ $jdbc_pool_li -ne $jdbc_pool_stat_li ]]; then
					echo "ERROR - Weblogic JDBC state is unknown"
					exit $STATE_UNKNOWN
				fi
				jdbc_pool_g=($( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME}:${PORT} $jdbcConnectionPoolRuntimeName |gawk -F: '{print $NF}'| sed -e 's/"//g' -e 's/ //g' ))
				jdbc_pool_stat_g=($( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME}:${PORT} $jdbcConnectionPoolRuntimeState |gawk -F: '{print $NF}'| sed -e 's/"//g' -e 's/ //g' ))
			else
				jdbc_pool_test=$( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME} $jdbcConnectionPoolRuntimeName )
				r1=$?
				if [[ $r1 -ne 0 ]] ; then
				...
				jdbc_pool_stat_li=$( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME} $jdbcConnectionPoolRuntimeState | wc -l)
				if [[ $jdbc_pool_li -ne $jdbc_pool_stat_li ]]; then
					echo "ERROR - Weblogic JDBC state is unknown"
					exit $STATE_UNKNOWN
				fi
				jdbc_pool_g=($( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME} $jdbcConnectionPoolRuntimeName |gawk -F: '{print $NF}'| sed -e 's/"//g' -e 's/ //g' ))
				jdbc_pool_stat_g=($( $SNMPWALK -v $VER -c ${COMMUNITY}@${TITLE1} ${HOSTNAME} $jdbcConnectionPoolRuntimeState |gawk -F: '{print $NF}'| sed -e 's/"//g' -e 's/ //g' ))
			fi
