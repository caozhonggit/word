...
#snmp_sco_disk_check.sh -H hostname/IP -C community -w warning -c critcal [-d diskname]
#This nagios plagin is finished by HongRui Wang at 2010-04-28
#2012-04-17 此版本支持监控某个挂载点，也支持监控所有的挂载点。当监控所有的挂载点时，只显示超过阀值的。
...
if [[ -n $HOSTNAME && -n $COMMUNITY && -n $WARN && -n $CRIT ]];then 
	#How many disk 
    disknum=$( $SNMPWALK -v 1 -c $COMMUNITY $HOSTNAME $hrStorageType |wc -l )
	r1=$?
	...
	if [[ $r1 -eq 0 && $r2 -eq 0 && $r3 -eq 0 && $r4 -eq 0 && $r5 -eq 0 ]];then					
		...
		if [[ -n $CHECKDISK ]];then
			...
			#因为挂载点会比磁盘数量少两个，且磁盘数量前两个总为内存和虚拟内存
			(( i=${i}+2 ))          
			...
			do	
				if [[ ${total_cnt_g[${j}]} -eq 0 ]];then
					...
					if [[ ${usage_g[${j}]} -le $WARN ]];then
						...
						(( warn_num=${warn_num}+1 ))
						printf "%s%s%s%s%s%s%s%s%%%s" " [WARN Mounted: " ${disk_mnt_g[${m}]} "   Total_MB: " ${total_M_g[${j}]} "   Avail_MB: " ${avail_M_g[${j}]} "   Usage: " ${usage_g[${j}]} "]"
					else
						(( crit_num=${crit_num}+1 ))
						printf "%s%s%s%s%s%s%s%s%%%s" " [CRIT Mounted: " ${disk_mnt_g[${m}]} "   Total_MB: " ${total_M_g[${j}]} "   Avail_MB: " ${avail_M_g[${j}]} "   Usage: " ${usage_g[${j}]} "]"
					fi
					#磁盘挂载点里不包含内存和虚拟内存，但是上述计算数组中都包含有这两项的值，所以当计算完后，需要对应挂载点名字时，要用数组中的序号减2
					#(( m=${j}-2 ))
					#printf "%s %s %sMB %sMB %s%% \n" ${diskname[$n]} ${diskmount[$n]} $volume $availab $diskusage | awk '{printf "%30s%20s%10s%10s%10s%\n",$1,$2,$3,$4,$5}'
					#printf "%-30s%10s%10s%10s%%\n" ${disk_mnt_g[${m}]} ${total_M_g[${j}]} ${avail_M_g[${j}]} ${usage_g[${j}]}
					(( j=${j}+1 ))
				fi
			done
