			...
			do	
				if [[ ${total_cnt_g[${j}]} -eq 0 ]];then
					...
					continue
                                        ...
                                        if [[ ${disk_mnt_g[${j}]} = "Physicalmemory" ]]; then
					  (( j=${j}+1 ))
                                          continue
                                        elif [[ ${disk_mnt_g[${j}]} = "Virtualmemory" ]]; then
					  (( j=${j}+1 ))
                                          continue
                                        elif [[ ${disk_mnt_g[${j}]} = "Memorybuffers" ]]; then
                                          (( j=${j}+1 ))
                                          continue
                                        elif [[ ${disk_mnt_g[${j}]} = "Cachedmemory" ]]; then
                                          (( j=${j}+1 ))
                                          continue
                                        elif [[ ${disk_mnt_g[${j}]} = "Sharedmemory" ]]; then
                                          (( j=${j}+1 ))
                                          continue
                                        elif [[ ${disk_mnt_g[${j}]} = "Swapspace" ]]; then
                                          (( j=${j}+1 ))
                                          continue
                                        else 
