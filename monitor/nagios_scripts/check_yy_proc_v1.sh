...
if [[ ${OK_ON_BOTH} == "OK" ]];then
	...
	if [[ ${OK_ON_BOTH} == "OK" ]];then
		...
		echo "check_nt Result from 10.1.5.128: $R_ON_YYHOST1" >> /usr/local/nagios/libexec/check_yy_proc.log
		echo "Not running on 10.1.5.128 : $UNRUN_ON_YYHOST1"  >> /usr/local/nagios/libexec/check_yy_proc.log
		echo "check_nt Result from 10.1.5.129: $R_ON_YYHOST2" >> /usr/local/nagios/libexec/check_yy_proc.log
		exit 2
	fi
fi
